...
# what's new
# In letor 1.0, the prediction file contains the prediction scores. Rank are generated according to
# the scores and ranking measures are calculated. However, if the ranking scores of two documents are
# identical. The measures may not stable.
# In this version, input <rank prediction file> contains the rank (start from 1 to n) of the documents in for queries.
# It will get the rank of documents directly from the input file and calculating the MAP and NDCG etc.
...
if($argc != 4)
{
		...
		print "argv[2]: rank prediction file\n";
...
{
	print "Invalid command line.\n";
	print "Usage: perl Eval-Rank.pl argv[1] argv[2] argv[3] argv[4]\n";
...
sub EvalQuery
{
    ...
    {
        my $qid = $qids[$i];
        my @docids = sort{$$pHash{$qid}{$a}{"pred"} <=> $$pHash{$qid}{$b}{"pred"}} keys(%{$$pHash{$qid}});

        # check if the input rank contain errors
        for(my $rank = 1; $rank <= $#docids; $rank ++)
        {
            my $curDid = $docids[$rank - 1];
            my $predRank = $$pHash{$qid}{$curDid}{"pred"};
            if ($predRank != $rank)
            {
                my $curLineNum = $$pHash{$qid}{$curDid}{"lineNum"};
                print "Rank input error for query $qid, doc $curDid, and line number $curLineNum\n";
                exit(-2);
            }
        }
...
sub ReadInputFiles
{
    ...
    while(defined(my $lnFea = <FIN_Feature>))
    {
        $lineNum ++;
        chomp($lnFea);
        my $predRank = <FIN_Pred>;
        chomp($predRank);
        if ( (!defined($predRank)) || !($predRank =~ m/^\d+$/))
        {
...
#        if ($lnFea =~ m/^([0-2]) qid\:(\d+).*?\#docid = (\d+)$/)
        if ($lnFea =~ m/^(\d+) qid\:([^\s]+).*?\#docid = ([^\s]+)$/)
        {
            ...
            $hsQueryDocLabelScore{$qid}{$did}{"pred"} = $predRank;
